...
class IdenticalQuadLayer(nn.Module):
    ...
    y=fc([x*x^T,x])
    '''
    def __init__(self,in_n,out_n,bias=False):
        super(IdenticalQuadLayer,self).__init__()
        n_fc_in=in_n*(in_n+3)
        n_fc_in>>=1
        self.idx=get_square_indices(in_n)
        self.fc=nn.Linear(n_fc_in,out_n,bias=bias)
    def forward(self,x):
        bs=x.size(0)
        x1=x.unsqueeze(2)
        x2=x.unsqueeze(1)
...
class NonIdenticalQuadLayer(nn.Module):
    ...
    '''
    def __init__(self,in_n1,in_n2=None,out_n=1,bias=False):
        super(NonIdenticalQuadLayer,self).__init__()
        if in_n2 is None:
            in_n2=in_n1
        self.fc=nn.Linear(in_n1*in_n2+in_n1+in_n2,out_n,bias=bias)
...
class QuadFCLayer(UnitLayer):
    def __init__(self, in_n1,in_n2=None,is_identical=True,out_n=1,bn=True,bn_track=True,activate='none',dropout=0.0,activation_args=None,bias=False,*args,**kwargs):
        ...
        if in_n2 is None:
            in_n2=in_n1
        fc=IdenticalQuadLayer(in_n1,out_n,bias=bias) if is_identical else NonIdenticalQuadLayer(in_n1,in_n2,out_n,bias=bias)
